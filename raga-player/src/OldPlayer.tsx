...
import LineDisplay from './LineOld';
...
export default function AppOld() {
  ...
  const parseSong = async (songname: string) => {
    ...
    const notesAndSands = musicalBars.map(bar => getSemitoneAndDurationForBar(bar, shruti, speed, mapping));
    const allSemitoneAndDurations = _.flatten(notesAndSands.map(notesAndSand => notesAndSand.sand));
